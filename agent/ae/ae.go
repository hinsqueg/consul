...
const (
	// serverUpIntv is the max time to wait before a full sync is triggered when a
	// consul server has been added to the cluster.
	serverUpIntv = 3 * time.Second

	// retryFailIntv is the min time to wait before a failed full sync is retried.
...
// Pause temporarily disables partial sync runs.
func (s *StateSyncer) Pause() {
	atomic.AddInt32(&s.paused, 1)
}

// Paused returns whether partial sync runs are temporarily disabled.
func (s *StateSyncer) Paused() bool {
	return atomic.LoadInt32(&s.paused) > 0
}

// Resume re-enables partial sync runs.
...
// triggerSync queues a partial sync run if one has not been triggered already.
